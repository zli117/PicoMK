...
enum {
  UP =
      TOTAL_BUILT_IN_KC,  // Avoid conflicting with the built-in custom keycodes
  DOWN,
  SELECT
};
...
static constexpr Keycode kKeyCodes[][CONFIG_NUM_PHY_ROWS][CONFIG_NUM_PHY_COLS] = {
  ...
  [1]={
    ...
    {K(K_ESC),   K(K_TAB),    K(K_Q),     K(K_W),     K(K_E),     K(K_R),     K(K_T),     K(K_Y),     K(K_U),     K(K_I),     K(K_O),     K(K_P),     K(K_BRKTL), K(K_BRKTR), K(K_BKSL)},
    {K(K_DEL),   K(K_CAPS),   K(K_A),     K(K_S),     K(K_D),     K(K_F),     K(K_G),     K(K_H),     K(K_J),     K(K_K),     K(K_L),     K(K_SEMIC), K(K_APST),  CK(SELECT)},
    {MO(3),      K(K_SFT_L),  K(K_Z),     K(K_X),     K(K_C),     K(K_V),     K(K_B),     K(K_N),     K(K_M),     K(K_COMMA), K(K_PERID), K(K_SLASH), K(K_SFT_R)},
    {CONFIG,     TG(2),       K(K_ALT_L), K(K_SPACE), ______,     ______,     K(K_ARR_L), CK(DOWN),   CK(UP),     K(K_ARR_R)},
    {CK(MSE_L),  CK(MSE_M),   CK(MSE_R)}
  },
  [2]={
    {CK(CONFIG_SEL)},
...
// Create custom key code handlers. See CustomKeycodeHandler in keyscan.h
...
class ConfigUpHandler : public CustomKeycodeHandler {
 public:
  ...
  std::string GetName() const override { return "Config Up"; }
};
...
class ConfigDownHandler : public CustomKeycodeHandler {
 public:
  ...
  std::string GetName() const override { return "Config Down"; }
};
...
class ConfigSelectHandler : public CustomKeycodeHandler {
 public:
  ...
  std::string GetName() const override { return "Config Select"; }
};

REGISTER_CUSTOM_KEYCODE_HANDLER(UP, true, ConfigUpHandler);
REGISTER_CUSTOM_KEYCODE_HANDLER(DOWN, true, ConfigDownHandler);
REGISTER_CUSTOM_KEYCODE_HANDLER(SELECT, true, ConfigSelectHandler);
